...
コンテナ内には .NET 8 SDK、Docker CLI、SQLite CLI が含まれます。Dev Container は `MailBatchSample/docker-compose.yml` と `.devcontainer/docker-compose.yml` を組み合わせた Docker Compose サービスとして起動し、検証用サービスと同じ Compose ネットワークに最初から参加します。Docker はホスト側 Docker を利用する設定のため、コンテナ内から `docker compose` を使って `MailBatchSample/docker-compose.yml` の検証環境を操作できます。

Dev Container 内から Docker Compose のサービスへ接続する場合は、`docker compose` でメールサーバと API を起動し、`mailreceiver-api:8080` や `mailserver:3025` / `mailserver:3143` のサービス名を使います。Dev Container を抜けてホスト OS のターミナルで確認する場合だけ、Compose が公開した `localhost` のポートを参照してください。
...
Dev Container 外のホスト OS で `dotnet run` や `curl` を実行する場合は、Compose の公開ポートを直接 `localhost` で参照できます。アプリの既定値は Dev Container から Compose サービス名へ接続する構成のため、ホスト OS で `dotnet run` する場合は次のように接続先を上書きしてください。
...
Dev Container は検証用サービスと同じ Compose ネットワークに参加済みです。メールサーバと連携先 API を Docker Compose で起動するには、Dev Container 内で次のコマンドをリポジトリルートから実行します。
...
```bash
docker compose -f MailBatchSample/docker-compose.yml ps
...
| 用途 | URL / 接続先 |
| --- | --- |
| SMTP | `mailserver:3025` |
| IMAP | `mailserver:3143` |
| MailReceiver.Api | `http://mailreceiver-api:8080` |
...
| 項目 | Dev Container / Compose 内実行時 | ホスト実行時 |
| --- | --- | --- |
...
```bash
curl http://mailreceiver-api:8080/api/received-mails/1
```
...
```bash
MAILSERVER_SMTP_HOST_PORT=2025 MAILSERVER_IMAP_HOST_PORT=2143 MAILRECEIVER_API_HOST_PORT=5500 \
  docker compose -f MailBatchSample/docker-compose.yml up -d --build mailserver mailreceiver-api
```

ホスト OS から公開ポートへ接続する場合は、`TestMailSender`、`MailBatch.Console`、確認用 `curl` の接続先も同じ値に合わせてください。Dev Container 内で Compose サービス名を使う場合は、ホスト側公開ポートの変更に追従する必要はありません。
...
- `nc -vz mailserver 3025` で SMTP ポートへ接続できることを確認してください。
- ホスト OS から実行して Compose の SMTP 公開ポートを変更している場合は、`TESTMAILSENDER_Smtp__Port` またはコマンドライン引数 `Smtp:Port` で `TestMailSender` の接続先を上書きしてください。
- 送信先ユーザーを変更している場合は、`MAILSERVER_USERS` と `Mail:To` の設定が一致していることを確認してください。

### バッチでメールを取得できない

- `curl --url "imap://mailserver:3143/INBOX" --user "test@example.local:password"` でメールボックスにメールが存在することを確認してください。
- 件名が既定の対象条件 `連携対象` を含むことを確認してください。対象外メールを投入した場合、バッチの取得件数は 0 件になります。
- ホスト OS から実行して Compose の IMAP 公開ポートを変更している場合は、`MAILBATCH_Imap__Port` またはコマンドライン引数 `Imap:Port` で `MailBatch.Console` の接続先を上書きしてください。

### API 連携または保存結果を確認できない

- `curl http://mailreceiver-api:8080/api/received-mails` で API 側に保存済みデータがあるか確認してください。
