...
Dev Container 内から Docker Compose のサービスへ接続する場合は、`scripts/dev-compose-up.sh` で Dev Container を Compose ネットワークへ接続し、`mailreceiver-api:8080` や `mailserver:3025` / `mailserver:3143` のサービス名を使います。Dev Container を抜けてホスト OS のターミナルで確認する場合だけ、Compose が公開した `localhost` のポートを参照してください。
...
メールサーバと連携先 API を Docker Compose で起動し、Dev Container を Compose ネットワークへ接続します。Dev Container 内では、次のラッパースクリプトをリポジトリルートから実行します。
...
```bash
# 1. メールサーバと API を起動し、Dev Container を Compose ネットワークへ接続する
./scripts/dev-compose-up.sh
...
```bash
MAILSERVER_SMTP_HOST_PORT=2025 MAILSERVER_IMAP_HOST_PORT=2143 MAILRECEIVER_API_HOST_PORT=5500 ./scripts/dev-compose-up.sh
```
