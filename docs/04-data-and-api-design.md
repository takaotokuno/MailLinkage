...
バッチ側は API 実行後に確定した事実を `mail-processing.db` の `api_execution_results` へ保存する。対象メール、実行・バッチ ID、エンドポイント、成否、HTTP ステータス、保存済み ID、移動後の UID / UIDVALIDITY、失敗レスポンスの要約、例外種別、開始・完了日時、所要時間を検索できる。IMAP サーバーが移動先 UID を返さない場合、移動後の UID / UIDVALIDITY は `NULL` とする。機微情報を含み得るリクエスト本文とレスポンス全文は DB に保存しない。
