...
| アプリ | 設定 | 例 |
| --- | --- | --- |
| MailBatch.Console | `Imap:Host` | Dev Container: `host.docker.internal` / ホスト OS: `localhost` / Compose 内: `mailserver` |
...
| MailBatch.Console | `Api:BaseUrl` | Dev Container: `http://host.docker.internal:5000` / ホスト OS: `http://localhost:5000` / Compose 内: `http://mailreceiver-api:8080` |
...
| TestMailSender | `Smtp:Host` | Dev Container: `host.docker.internal` / ホスト OS: `localhost` / Compose 内: `mailserver` |
...
| 項目 | Dev Container 実行時 | ホスト OS 実行時 | Compose 内実行時 |
| --- | --- | --- | --- |
| SMTP ホスト | `host.docker.internal` | `localhost` | `mailserver` |
| SMTP ポート | `1025` | `1025` | `3025` |
| IMAP ホスト | `host.docker.internal` | `localhost` | `mailserver` |
| IMAP ポート | `1143` | `1143` | `3143` |
| ユーザー名 | `test@example.local` | `test@example.local` | `test@example.local` |
| パスワード | `password` | `password` | `password` |
| メールボックス | `INBOX` | `INBOX` | `INBOX` |
