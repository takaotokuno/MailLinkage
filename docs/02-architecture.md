...
flowchart LR
    ...
    Batch -->|台帳・履歴| State[(SQLite\nlogs/mail-processing.db)]
    Batch -->|SMTP 通知| MailServer
...
設定値は `appsettings.json` と環境変数で管理する。Docker Compose から起動する場合は環境変数で上書きできるようにする。MailBatch.Console は `MAILBATCH_` プレフィックス付き環境変数を読み込み、例えば `MAILBATCH_Imap__Password` のように `__` で階層を表現する。

シークレット供給は特定基盤に固定しない。通常は設定ファイル、環境変数、コマンドライン引数の標準 `IConfiguration` プロバイダーで完結させ、Azure Key Vault を使う環境だけ `AzureKeyVault:Enabled=true` と `AzureKeyVault:VaultUri` を設定して Key Vault プロバイダーを追加する。Key Vault 追加後にも環境変数とコマンドライン引数を再追加するため、コンテナ動作確認や障害時の一時対応では環境変数で上書きできる。
...
| MailBatch.Console | `Api:ApiKey` | API要求の `X-API-Key` ヘッダーに設定するキー（本番環境では環境変数 `MAILBATCH_Api__ApiKey` または Key Vault から供給） |
| MailBatch.Console | `MailSearch:SubjectContains` | `連携対象` |
| MailBatch.Console | `MailSearch:From` | `sender@example.local` |
| MailBatch.Console | `Processing:ProcessedMailbox` | `Processed` |
| MailBatch.Console | `Processing:ErrorMailbox` | `Error` |
| MailBatch.Console | `Processing:RequestQueueCapacity` | `100` |
| MailBatch.Console | `Api:RetryCount` / `Imap:RetryCount` | `3` |
| MailBatch.Console | `Notification:SmtpHost` | `mailserver` |
