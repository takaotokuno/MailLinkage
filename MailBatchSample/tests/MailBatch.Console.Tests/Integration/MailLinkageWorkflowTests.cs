...
public sealed class MailLinkageWorkflowTests : IDisposable
{
    ...
    /// <summary>
    /// 正常メールが一度だけAPI連携され再処理されないことを確認する。
    /// </summary>
    /// <remarks>
    /// 前提・入力: 有効なKey行を持つ未処理メールを受信箱に配置してバッチを2回実行する。<br/>
    /// 期待結果: API送信は初回の1件のみで、メールは処理済みへ移動し、2回目には再送信されない。<br/>
    ...
    /// <summary>
    /// 業務形式不正メールをAPIへ送らず受信箱から除外することを確認する。
    /// </summary>
    /// <remarks>
    /// 前提・入力: Key行を持たないメールを受信箱に配置する。<br/>
    /// 期待結果: API送信は0件で、対象メールはエラーフォルダーへ移動する。<br/>
    ...
    /// <summary>
    /// API拒否後のメールを次回実行で再試行できることを確認する。
    /// </summary>
    /// <remarks>
    /// 前提・入力: 初回は拒否し次回は成功するAPIと、有効な対象メールを用意する。<br/>
    /// 期待結果: 初回はエラーへ移動し、次回は同じメールを再送信して処理済みへ移動する。<br/>
