...
public sealed class SqliteMailProcessingStoreTests : IDisposable
{
    ...
    /// <summary>
    /// 処理済み記録と移動失敗記録が再生成後も保持されることを確認する。
    /// </summary>
    /// <remarks>
    /// 前提・入力: 処理済みIDとエラー移動失敗IDを保存し、ストアを作り直す。<br/>
    /// 期待結果: 新しいストアから処理済みIDと移動失敗の種別・IDを取得できる。<br/>
    ...
    /// <summary>
    /// 同一失敗の再記録で初回日時を保持できることを確認する。
    /// </summary>
    /// <remarks>
    /// 前提・入力: 同じメールIDの移動失敗を時間を空けて2回追加する。<br/>
    /// 期待結果: CreatedAtUtcは初回値のまま、LastFailedAtUtcだけが新しい日時になる。<br/>
    ...
    /// <summary>
    /// 復旧再失敗時に最終失敗日時だけ更新することを確認する。
    /// </summary>
    /// <remarks>
    /// 前提・入力: 保存済み移動失敗に対して復旧失敗を記録する。<br/>
    /// 期待結果: CreatedAtUtcは変わらず、LastFailedAtUtcだけが後の日時へ更新される。<br/>
    ...
    /// <summary>
    /// 旧スキーマの失敗日時を新しい日時列へ移行できることを確認する。
    /// </summary>
    /// <remarks>
    /// 前提・入力: 単一の旧timestamp列を持つ移動失敗レコードをSQLiteに作成する。<br/>
    /// 期待結果: CreatedAtUtcとLastFailedAtUtcの両方に旧timestamp値が設定される。<br/>
